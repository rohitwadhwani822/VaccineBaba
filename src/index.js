...
var server = app.listen(port, () => {
  console.log('Server is started on 127.0.0.1:'+ port)
})
    ...
    var getSessionData = async (param_district_id,param_chat_id,param_date)=>{
      ...
      try {
        res = await axios.get('https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id='+param_district_id+'&'+'date='+param_date,{
            headers:{
                "User-Agent":"Mozilla/5.0 (X11; Linux x86_64) Mozilla/5.0 (X11; Linux x86_64) Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.93 Safari/537.36"
        ...
        for(var i=0;i<res.data.centers.length;i++){
            ...
            for(var j=0;j<raw_center.sessions.length;j++){
                ...
                if((age_limit<45 || (age_limit>=45 && (param_district_id==363 || param_district_id==392 || param_district_id==395))) && available_capacity>0){
